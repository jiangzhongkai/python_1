...
class SGA(object):
    ...
    def reshape_weight_bias(self,individuals):
        """
        :param indivduals:
        :return:
        """
        w1 = individuals[0:self.compute_param_num()[1]]
        b1 = individuals[self.compute_param_num()[1]:self.compute_param_num()[0]]
    ...
    #对每一个个体的适应度进行求值
    def fitness_fun(self,individuals):
        """
        ...
        """
        #step_1:提取参数,w1,b1
        new_w1,new_b1=self.reshape_weight_bias(individuals)
        ...
        for i in range(epoch):
            for start, end in zip(range(0, len(train_x), batch_size),
                ...
                fitness=sess.run(cost, feed_dict={X: train_x[start:end],Y: train_y[start:end]})  # 分离出batchsize个数据去迭代运算
...
if __name__=="__main__":
    ...
    for i in range(pop_size):
        ...
        #计算每个个体的适应度值
        individuals["fitness"][0][i]=sga.fitness_fun(indivaduals=x)  #适应度值
    ...
    for i in range(max_gen):
        ...
        for j in range(pop_size):
            ...
            individuals["fitness"][0][j]=sga.fitness_fun(indivaduals=x)
