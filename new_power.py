...
在power_demand2的基础上自己重新写了一个归一化函数和逆归一化函数
...
def createDataSet(filename):
    ...
    dataSet = []
    for i in range(0, len(data) - 672,
                   672):  # one hour is 4 point, then a day is need 24*4=96 point, so a week is 96*7=672 point
...
def downSample(dataSet):
    ...
    for i in range(len(dataSet)):
        line = np.ones([int(672 / 8)])  # 创建一个数组来装替换后一周的数据，故672/8
        j = 0
...
def split_data(dataSet):
    ...
    # print dataSet.shape
    timesteps = 12
    data_x = [dataSet[i:i + timesteps] for i in range(len(dataSet) - timesteps - 1 + 1)]
    data_x = np.array(data_x)
    data_y = [dataSet[i + timesteps] for i in range(len(dataSet) - timesteps)]
...
class Config(object):
    def __init__(self, train_x):
        ...
        self.biases = {
            'hidden': tf.Variable(tf.random_normal([self.hidden_two])),
...
def LSTM_Network(input_data, config):
    ...
    # 处理成一个一个时间步的输入到模型的数据
    lstm_cell1 = tf.contrib.rnn.BasicLSTMCell(config.hidden_nums, forget_bias=1.0, state_is_tuple=True)
    # dropout lstm_cell2
    lstm_cell2 = tf.contrib.rnn.BasicLSTMCell(config.hidden_two, forget_bias=1.0, state_is_tuple=True)
    # lstm_cell2 = tf.nn.rnn_cell.DropoutWrapper(lstm_cell2, config.keep_prob)
    # stack lstm cell
    stack_lstm = tf.contrib.rnn.MultiRNNCell([lstm_cell1, lstm_cell2], state_is_tuple=True)

    outputs, _ = tf.nn.static_rnn(stack_lstm, input_data, dtype=tf.float32)
...
if __name__ == '__main__':
    ...
    # saver.restore(sess, "./power/model.ckpt")
    #
    test_losses = []
    train_losses = []
    ...
    for i in range(epoch):
        ...
        # loss1 = sess.run(cost, feed_dict = {X: train_x, Y: train_y})
        if (i + 1) % 100 == 0:
    ...
    save_path = saver.save(sess, "./power1.2/model.ckpt")  # 保存模型
    # np.savetxt('test_result3.txt',test_result)
    # np.savetxt('train_result3.txt',train_result)
